...
export function AssetsOverview({
  ...
  return (
    <Card className='h-full'>
      <h3 className='text-lg font-bold text-earth dark:text-white flex items-center gap-2 mb-6'>
        ...
        <div className='relative'>
          <div className='w-40 h-40 rounded-full border-[16px] border-primary/20 flex items-center justify-center'>
            <div className='text-center'>
              <span className='text-2xl font-black text-earth dark:text-white'>{totalValue}</span>
              <p className='text-[10px] text-earth/40 dark:text-slate-500 uppercase tracking-widest'>
                Total Value
              </p>
            </div>
          </div>
      ...
      <div className='space-y-4'>
        {categories.map((category, index) => (
          <div key={`category-${index}-${category.name}`} className='flex items-center gap-3'>
            ...
            <span className='text-xs text-earth/60 dark:text-slate-400 flex-1'>
              {category.name}
            </span>
            <span className='text-xs font-bold text-earth dark:text-white'>
              {category.percentage}%
            </span>
          </div>
        ))}
